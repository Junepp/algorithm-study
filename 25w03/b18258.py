...
def deq_processor(deq, command):
    ...
    if command[0] == "push":
        ...
        response = deq.popleft() if deq else -1
        return response
        ...
        response = 1 if not deq else 0
        return response

    elif command[0] == "front":
        response = deq[0] if deq else -1
        return response
        
    elif command[0] == "back":
        response = deq[-1] if deq else -1
        return response
...
for line in lines[1:]:
    ...
    response = deq_processor(deq, command)
